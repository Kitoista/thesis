...
class Filler:
    ...
    def __call__(self, newState, pos, color):
        ...
        window_x = (max(0, pos[0] - window_x_size), min(newState.shape[0] - 1, pos[0] + window_x_size))
        window_y = (max(0, pos[1] - window_y_size), min(newState.shape[1] - 1, pos[1] + window_y_size))
